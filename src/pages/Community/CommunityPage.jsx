...
const STATS = [
  ...
  { value: 62, label: 'Community chapters', icon: <FiMapPin />, hint: 'Volunteer run groups, hosted online.' },
...
const EVENTS = [
  ...
  { date: '22 Aug', title: 'Community meetup', city: 'Online', type: 'Meetup', seats: '38 seats left', desc: 'Talks from the payments team, then an open floor for questions.' },
  ...
  { date: '19 Sep', title: 'Developer day Business Platform', city: 'Online', type: 'Conference', seats: '64 seats left', desc: 'API deep dives, webhook patterns and a live migration clinic.' },
  { date: '02 Oct', title: 'Community design jam', city: 'Online', type: 'Meetup', seats: '25 seats left', desc: 'Redesign a real KT surface alongside the design team, live.' },
  { date: '17 Oct', title: 'Privacy policy roundtable', city: 'Online', type: 'Roundtable', seats: 'Invite only', desc: 'Regulators, researchers and our policy team, on the record.' },
]
...
const AMBASSADOR_PERKS = [
  ...
  { icon: <FiHeart />, title: 'Event support', desc: 'Hosting and production costs covered for chapter meetups, plus swag that people actually keep.' },
...
const FAQS = [
  { q: 'Is the community run by KT Messenger?', a: 'The forums and official channels are ours and staffed by employees. Community chapters are volunteer run we fund and support them but do not control what they discuss.', tag: 'Structure' },
  ...
  { q: 'Can I start a chapter of my own?', a: 'Yes. Apply through the ambassador programme with a rough plan for the first three meetups and we will cover the hosting costs.', tag: 'Chapters' },
...
export function CommunityPage() {
  ...
  const toggleEvent = (title) => {
    ...
    setToast(joined ? `${t('Left')} ${t(title)}.` : `${t('You are on the list for')} ${t(title)}.`)
  }
  ...
  return (
    <MainLayout>
      <PageHero
        badge={
          <>
            <FiUsers /> {t('480k members · 62 chapters')}
          </>
        }
        title="KT"
        highlight={t('Community')}
        description={t('Forums, community chapters, ambassadors and a beta programme run in the open, with moderation that explains itself.')}
        actions={
          <>
            <Button size="lg" variant="white" onClick={() => document.getElementById('forums')?.scrollIntoView({ behavior: 'smooth' })}>
              {t('Browse the forums')} <FiChevronRight />
            </Button>
          </>
        }
        chips={[
          { icon: <FiShield />, label: t('Transparent moderation') },
          { icon: <FiGlobe />, label: t('28 languages covered') },
          { icon: <FiCalendar />, label: t('26 events this year') },
        ]}
        aside={
          <div className="rounded-[28px] border border-white/10 bg-white/[0.04] p-6 shadow-2xl backdrop-blur-xl sm:p-8">
            <span className="text-[11px] font-black uppercase tracking-[0.16em] text-sky-300">{t('Next up')}</span>
              ...
              {EVENTS.slice(0, 3).map((event) => (
                <li key={event.title} className="flex items-start gap-3 rounded-2xl border border-white/10 bg-white/[0.03] p-3.5">
                  ...
                  <span className="min-w-0">
                    <span className="block truncate text-xs font-bold text-white">{t(event.title)}</span>
                    <span className="block truncate text-[10px] font-semibold text-slate-400">
                      {t(event.city)} · {t(event.type)}
                    </span>
                  </span>
                </li>
              ))}
            ...
            <p className="mt-5 border-t border-white/10 pt-4 text-[11px] leading-relaxed text-slate-400">
              {t('Meetups, workshops and AMAs are free. Chapter events are volunteer run and funded by us.')}
            </p>
          </div>
        }
      />
      ...
      <div id="forums" className="scroll-mt-36">
        ...
        <Section className="border-y border-line bg-cream dark:bg-cream-2">
          <SectionHead
            eyebrow={`${filteredThreads.length} ${filteredThreads.length === 1 ? t('thread') : t('threads')}`}
            title={category === 'All' ? t('Community forums') : t(category)}
            description={t('Answered threads are marked by a moderator. Criticism stays up only spam and unsafe content is removed.')}
          />
          ...
          {filteredThreads.length === 0 ? (
            <div className="mt-12">
              <EmptyState
                icon={<FiSearch />}
                title={t('No threads match')}
                description={t('Try another category, or clear the search to see all 12 threads.')}
                action={
              ...
              <ul className="divide-y divide-line">
                {filteredThreads.map((thread) => (
                  <li key={thread.id}>
                    <button
                      type="button"
                      onClick={() => setToast(`${t('Opening')} “${t(thread.title)}” ${t('in the forums.')}`)}
                    ...
                    >
                      ...
                      <span className="min-w-0 flex-1">
                        <span className="flex flex-wrap items-center gap-2">
                          <span className="text-sm font-extrabold text-ink">{t(thread.title)}</span>
                        ...
                        <span className="mt-1 block text-[11px] font-semibold text-muted">
                          {t(thread.category)} · {t('by')} {thread.author}
                        </span>
                      </span>

                      <span className="hidden shrink-0 text-right sm:block">
                        <span className="block text-sm font-black text-ink">{thread.replies}</span>
                        <span className="block text-[10px] font-bold uppercase tracking-wide text-muted">{t('replies')}</span>
                      </span>

                      <span className="hidden shrink-0 text-right md:block">
                        <span className="block text-sm font-black text-ink">{thread.views}</span>
                        <span className="block text-[10px] font-bold uppercase tracking-wide text-muted">{t('views')}</span>
                      </span>
      ...
      <Section id="events" className="scroll-mt-36 bg-surface">
        <SectionHead
          eyebrow={t('Events')}
          title={t('Twenty six gatherings this year')}
          description={t('Meetups, workshops and AMAs. Free to attend tap to add yourself to the list.')}
        />
                ...
                <article className="flex h-full flex-col rounded-[24px] border border-line bg-cream p-6 shadow-soft transition-all duration-300 hover:-translate-y-1 hover:shadow-card dark:bg-cream-2">
                  <div className="flex items-start gap-4">
                    ...
                    <div className="min-w-0">
                      ...
                      <h3 className="mt-1.5 text-base font-extrabold leading-snug text-ink">{t(event.title)}</h3>
                    </div>
                  </div>

                  <p className="mt-4 flex-1 text-sm leading-relaxed text-body">{t(event.desc)}</p>

                  <div className="mt-5 flex items-center justify-between gap-3 border-t border-line pt-4">
                    <span className="min-w-0">
                      <span className="flex items-center gap-1.5 text-[11px] font-bold text-muted">
                        <FiMapPin className="shrink-0" /> {t(event.city)}
                      </span>
                      <span className="mt-0.5 block text-[11px] font-semibold text-muted">{t(event.seats)}</span>
                    </span>
      ...
      <Section id="ambassadors" className="scroll-mt-36 border-y border-line bg-cream dark:bg-cream-2">
        <SectionHead
          eyebrow={t('Ambassadors')}
          title={t('340 people answering questions in 28 languages')}
          description={t('Volunteers who help in the forums and run community chapters. Here is what they get for it.')}
        />
        ...
        <Reveal from="up" className="mx-auto mt-12 max-w-3xl rounded-[24px] border border-line bg-surface p-6 text-center sm:p-8">
          <h3 className="text-lg font-extrabold text-ink">{t('Want to join them?')}</h3>
          <p className="mx-auto mt-2 max-w-xl text-sm leading-relaxed text-body">
            {t('Be helpful in the forums for a few months, then apply. We look at consistency and tone rather than post count and every applicant gets a written answer either way.')}
          </p>
          <div className="mt-6 flex flex-wrap justify-center gap-3">
            <Button onClick={() => setToast(t('Ambassador application thread opened.'))}>
              {t('Apply to the programme')} <FiAward />
            </Button>
            <Button variant="secondary" onClick={() => { navigate('/contact'); window.scrollTo(0, 0) }}>
              {t('Ask a question first')}
            </Button>
          </div>
        </Reveal>
      </Section>
      ...
      <Section id="beta" className="scroll-mt-36 bg-surface">
        <div className="grid gap-10 lg:grid-cols-2 lg:items-center">
          <SectionHead
            align="left"
            eyebrow={t('Beta programme')}
            title={t('Run the next release two versions early')}
            description={t('Beta slots are capped per platform so feedback stays readable. You can leave at any point without losing data the vault format is forwards and backwards compatible.')}
          >
            ...
            <div className="mt-8 flex flex-wrap gap-3">
              <Button onClick={() => setToast(t('Beta slot reserved check your chat for the build link.'))}>
                {t('Join the beta')} <FiZap />
              </Button>
            </div>
          </SectionHead>
            ...
            <div className="grid gap-4 sm:grid-cols-2">
              {[
                ...
                <div key={item.platform} className="rounded-[24px] border border-line bg-cream p-5 shadow-soft dark:bg-cream-2">
                  ...
                  <p className="mt-0.5 text-[11px] font-bold text-muted">{item.slots} {t('slots filled')}</p>
                  ...
                  <p className="mt-2 text-[11px] font-black text-muted">{item.pct}% {t('full')}</p>
                </div>
              ))}
            </div>
          </Reveal>
      ...
      <Section id="guidelines" className="scroll-mt-36 border-y border-line bg-cream dark:bg-cream-2">
        <SectionHead
          ...
          title={t('Six rules, and how we enforce them')}
          description={t('Every removal comes with a stated reason and an appeal route. Moderator actions are published in aggregate each quarter.')}
        />
          ...
          {GUIDELINES.map((rule, index) => (
            <Reveal key={rule.title} from="up" delay={Math.min(index * 0.05, 0.25)} className="h-full">
              <div className="flex h-full flex-col rounded-[24px] border border-line bg-surface p-6 shadow-soft">
                ...
                <h3 className="mt-4 text-base font-extrabold leading-snug text-ink">{t(rule.title)}</h3>
                <p className="mt-2 flex-1 text-sm leading-relaxed text-body">{t(rule.desc)}</p>
              </div>
            </Reveal>
          ))}
        </div>
      ...
      <Section id="faq" container={false} className="scroll-mt-36 bg-surface">
        <Container maxW="max-w-3xl">
          <SectionHead eyebrow={t('FAQ')} title={t('How the community works')} />
          <div className="mt-12">
      ...
      <CtaBand
        eyebrow={t('Join in')}
        title={t('480,000 people, one shared rule: be useful')}
        description={t('Ask a question, answer one, run a meetup for your own group, or just follow along on the channels above.')}
        actions={
          <>
            <Button size="lg" variant="white" onClick={() => document.getElementById('forums')?.scrollIntoView({ behavior: 'smooth' })}>
              {t('Browse the forums')}
        ...
        points={[t('Transparent moderation'), t('62 community chapters'), t('Free events'), t('Criticism stays up')]}
      />
