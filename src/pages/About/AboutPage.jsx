...
const NAV_ITEMS = [
  ...
  { id: 'offices', label: 'Our team', icon: <FiMapPin /> },
]
...
const STATS = [
  ...
  { value: 1200, label: 'People on the team', icon: <FiAward />, hint: 'A fully distributed, remote first team.' },
...
const TIMELINE = [
  {
    ...
    desc: 'Four engineers, one question: could a forward secret encrypted messenger feel as effortless as an unencrypted one?',
  },
  {
    year: '2019',
...
const OFFICES = [
  { city: 'Engineering Hub', country: 'Remote first team', team: 'Platform · Payments', flag: '🛠️' },
  { city: 'Protocol Hub', country: 'Remote first team', team: 'Protocol · Security', flag: '🔐' },
  { city: 'Design Studio', country: 'Remote first team', team: 'Design · Research', flag: '🎨' },
  { city: 'Operations Hub', country: 'Remote first team', team: 'Support · Reliability', flag: '⚙️' },
  { city: 'Growth Hub', country: 'Remote first team', team: 'Growth · Lifecycle', flag: '📈' },
  { city: 'Partnerships Hub', country: 'Remote first team', team: 'Partnerships · Business', flag: '🤝' },
  { city: 'Policy Desk', country: 'Remote first team', team: 'Policy · Legal', flag: '⚖️' },
  { city: 'Fully Distributed', country: 'Remote first team', team: 'Everyone, every timezone', flag: '🌐' },
]
  ...
  return (
    <MainLayout>
      <PageHero
        badge={
          <>
            <FiFlag /> {t('Independent since 2018')}
          </>
        }
        title={t('About')}
        highlight="KT Messenger"
        description={t('We build private messaging for two billion people and we fund it with subscriptions, not with what you say to your family.')}
        actions={
          <>
            <Button size="lg" variant="white" onClick={() => document.getElementById('mission')?.scrollIntoView({ behavior: 'smooth' })}>
              {t('Read our mission')} <FiChevronRight />
            </Button>
        ...
        chips={[
          { icon: <FiLock />, label: t('End to end encrypted by default') },
          { icon: <FiEye />, label: t('No advertising business') },
          { icon: <FiGlobe />, label: t('180 countries') },
        ]}
        aside={
          <div className="rounded-[28px] border border-line dark:border-white/10 bg-surface dark:bg-white/[0.04] p-6 shadow-2xl backdrop-blur-xl sm:p-8">
            <span className="text-[11px] font-black uppercase tracking-[0.16em] text-brand-strong dark:text-sky-300">{t('What we refuse to do')}</span>
            ...
            <p className="mt-6 border-t border-line dark:border-white/10 pt-5 text-xs leading-relaxed text-muted dark:text-slate-400">
              {t('These four lines have been in our charter since 2019 and require a board supermajority to change.')}
            </p>
          </div>
        }
      />
      ...
      <Section id="mission" className="scroll-mt-36 bg-surface">
        <div className="grid gap-12 lg:grid-cols-2 lg:items-center">
          <SectionHead
            align="left"
            eyebrow={t('Mission')}
            title={t('Private conversation should be the default, not the upgrade')}
            description={t("Most of the world's messaging runs on infrastructure that reads it. We think that is a design choice, not a law of physics so we set out to prove an encrypted messenger could also be the fastest and friendliest one on the phone.")}
          >
            <div className="mt-8 space-y-4">
              {[
                ...
                <div key={item.title} className="rounded-[22px] border border-line bg-cream p-5 dark:bg-cream-2">
                  ...
                  <p className="mt-1.5 text-sm leading-relaxed text-body">{t(item.desc)}</p>
                </div>
              ))}
            </div>
          </SectionHead>
          ...
          <Reveal from="scale" delay={0.08}>
            <div className="rounded-[28px] border border-line bg-cream p-8 shadow-card dark:bg-cream-2">
              ...
              <blockquote className="-mt-4 text-lg font-semibold leading-relaxed text-ink sm:text-xl">
                {t("We had one rule when we started: if a feature only works by reading people's messages, it does not get built. Eight years later that rule has cost us a lot of revenue and exactly zero customers.")}
              </blockquote>
              <figcaption className="mt-6 flex items-center gap-3 border-t border-line pt-5">
                ...
                <span>
                  ...
                  <span className="block text-xs font-semibold text-muted">{t('Co founder & CEO')}</span>
                </span>
              </figcaption>
            </div>
          </Reveal>
      ...
      <Section id="story" className="scroll-mt-36 border-y border-line bg-cream dark:bg-cream-2">
        <SectionHead
          eyebrow={t('Our story')}
          title={t('Eight years, one constraint')}
          description={t('Every milestone below had to clear the same bar: does it work without us reading anything?')}
        />
          ...
          {TIMELINE.map((item, index) => (
            <Reveal as="li" key={item.year} from="up" delay={index * 0.05} className="relative flex gap-5 pb-10 last:pb-0 sm:gap-7">
              ...
              <div className="min-w-0 flex-1 rounded-[22px] border border-line bg-surface p-5 shadow-soft">
                <h3 className="text-base font-extrabold text-ink">{t(item.title)}</h3>
                <p className="mt-1.5 text-sm leading-relaxed text-body">{t(item.desc)}</p>
              </div>
            </Reveal>
          ))}
      ...
      <Section id="values" className="scroll-mt-36 bg-surface">
        <SectionHead
          eyebrow={t('Values')}
          title={t('Six principles we actually argue about')}
          description={t('These are the tiebreakers in real product reviews, not poster copy.')}
        />
      ...
      <Section id="team" className="scroll-mt-36 border-y border-line bg-cream dark:bg-cream-2">
        <SectionHead
          eyebrow={t('Leadership')}
          title={t('Who makes the calls')}
          description={t('A small team with long tenure the median leader here has been building this for six years.')}
        />
          ...
          {LEADERSHIP.map((person, index) => (
            <Reveal key={person.name} from="up" delay={Math.min(index * 0.04, 0.24)} className="h-full">
              <div className="flex h-full flex-col rounded-[24px] border border-line bg-surface p-5 shadow-soft transition-all duration-300 hover:-translate-y-1 hover:shadow-card">
                ...
                <p className="text-[11px] font-bold text-brand-ink">{t(person.role)}</p>
                <p className="mt-2 flex-1 text-xs leading-relaxed text-body">{t(person.focus)}</p>
              </div>
            </Reveal>
          ))}
        </div>
      ...
      <Section id="offices" className="scroll-mt-36 bg-surface">
        <SectionHead
          eyebrow={t('Our team')}
          title={t('A remote first team, and a lot of kitchen tables')}
          description={t('The whole company is distributed by default. Where you sit has never decided what you get to work on.')}
        />
            ...
            <Reveal key={office.city} from="up" delay={Math.min(index * 0.04, 0.24)} className="h-full">
              <div className="flex h-full items-start gap-4 rounded-[24px] border border-line bg-cream p-5 shadow-soft dark:bg-cream-2">
                ...
                <div className="min-w-0">
                  <h3 className="truncate text-sm font-extrabold text-ink">{t(office.city)}</h3>
                  <p className="truncate text-[11px] font-semibold text-muted">{t(office.country)}</p>
                  <p className="mt-1.5 text-[11px] font-bold text-brand-ink">{t(office.team)}</p>
                </div>
              </div>
            </Reveal>
          ))}
      ...
      <CtaBand
        eyebrow={t('Join us')}
        title={t('We are hiring across engineering, design and policy')}
        description={t('If the constraint in our mission sounds like the interesting part of the job rather than the annoying part, we should talk.')}
        actions={
          <>
        ...
        points={[t('No ads charter'), t('Published audits'), t('Remote first team'), t('Founded 2018')]}
      />
