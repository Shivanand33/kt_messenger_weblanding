...
export function GroupsLoopVideo({ className = '' }) {
  ...
  return (
    <PhoneVideoFrame
      title={t('Groups, Polls & Communities')}
      progress={progress}
      isPlaying={isPlaying}
    ...
    >
      ...
      <div className="flex items-center justify-between border-b border-white/10 pb-2">
        <div className="flex items-center gap-2">
          <img src={img(groupImg)} alt={imgAlt(groupImg, t('Group'))} className="h-7 w-7 rounded-full object-cover border border-brand-strong" />
          <div>
        ...
        {phase === 0 && (
          ...
          >
            <div className="flex items-center justify-between text-[11px] font-bold text-brand-ink">
              <span className="flex items-center gap-1"><FiBarChart2 className="text-brand-strong" /> {t('Poll: Dinner location?')}</span>
              <span className="text-[9px] text-slate-400">{voteCount} {t('votes')}</span>
            </div>
            <div className="relative overflow-hidden rounded-lg bg-slate-900 p-2 border border-brand-strong/40">
              ...
              <div className="relative flex items-center justify-between text-[10px] font-medium text-slate-200">
                <span className="flex items-center gap-1"><FiCheckCircle className="text-brand-strong text-xs" /> {t('Beach Pizzeria 🍕')}</span>
        ...
        {phase === 1 && (
          ...
          >
            <div className="flex items-center justify-between text-brand-ink font-bold text-[10px]">
              <span className="flex items-center gap-1"><FiCalendar className="text-brand-strong" /> {t('Group Event')}</span>
        ...
        {phase === 2 && (
          ...
          >
            <div className="flex items-center justify-between font-bold text-[10px] text-brand-ink">
              <span className="flex items-center gap-1"><FiUsers className="text-brand-strong" /> {t('Community Hub')}</span>
              <span className="text-[9px] text-slate-400">{t('5 Sub Groups')}</span>
            </div>
            <div className="rounded-lg bg-slate-900 p-1.5 border border-slate-800 flex items-center justify-between text-[10px]">
              ...
              <span className="text-[8px] text-brand-strong font-semibold font-mono">{t('1,024 Joined')}</span>
            </div>
            <div className="rounded-lg bg-slate-900 p-1.5 border border-slate-800 flex items-center justify-between text-[10px]">
              <span>{t('💬 General Chat')}</span>
              <span className="text-[8px] text-slate-400 font-mono">{t('342 Joined')}</span>
            </div>
          </motion.div>
        )}
      </div>
      ...
      <div className="rounded-xl bg-slate-950 p-1.5 text-center text-[9px] font-bold text-brand-ink border border-slate-800">
        {t('Live polls & event RSVPs update in real time')}
      </div>
    </PhoneVideoFrame>
  )
}
